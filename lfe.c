...
void lf_scalefactor (double lfe_sample[12], unsigned int *lfe_scalar)
{
  int j;
  double s;

  for (j = 1, s = fabs (lfe_sample[0]); j < 12; j++)
    if (fabs (lfe_sample[j]) > s)
      s = fabs (lfe_sample[j]);
...
void II_encode_lfe_scale (unsigned int lfe_scalar, Bit_stream_struc * bs)
{
  putbits (bs, lfe_scalar, 6);
}


void lfe_allocation (unsigned int *lfe_alloc, int *adb)
{				/* 70 */
  int nbal, bspl, bscf;
...
void II_lfe_quantisation (unsigned int lfe_scalar,
...
void II_encode_lfe_alloc (unsigned int lfe_alloc,
...
void II_lfe_sample_encoding (unsigned int lfe_sbband[12],
			     unsigned int lfe_alloc,
			     frame_params * fr_ps, Bit_stream_struc * bs)
{
  unsigned int s;
...
void lfe_filter (double **buffer, double lfe_sample[12])
{
  static double c;
  static double d[8];
  ...
  for (i = 0; i < 1152; i++) {
    ...
    for (k = 0; k < 8; k++) {
      v0[k] = sambuf[i];
      sambuf[i] =
	v0[k] * a0[k] + v1[k] * a1[k] + v2[k] * a2[k] - w1[k] * b1[k] -
	w2[k] * b2[k];
      v2[k] = v1[k];
      v1[k] = v0[k];
      w2[k] = w1[k];
